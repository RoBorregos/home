    ...
    def run(self) -> None:
        ...
        while not rospy.is_shutdown():
            if self.current_state == TaskManagerServer.TASK_STATES["GET_BAG"]:
                ...
                self.current_state = TaskManagerServer.TASK_STATES["ASK_FOR_BAG"]
            
            if self.current_state == TaskManagerServer.TASK_STATES["ASK_FOR_BAG"]:
                rospy.loginfo("[INFO] Asking for bag...")
                self.subtask_manager["hri"].speak(F"I saw you pointed at the {self.bag.name} bag, can you give it to me? I'll wait for you to come closer so I open my gripper")
                self.subtask_manager["manipulation"].go_to_joint_position("RECEIVE_JOINT_POSITION")
                self.current_state = TaskManagerServer.TASK_STATES["WAITING_FOR_PERSON"]
                
            if self.current_state == TaskManagerServer.TASK_STATES["WAITING_FOR_PERSON"]:
                rospy.loginfo("[INFO] Waiting for person...")
                self.subtask_manager["vision"].wait_for_person()
                rospy.loginfo("[INFO] Person found")
                self.current_state = TaskManagerServer.TASK_STATES["RECEIVE_BAG"]
            
            if self.current_state == TaskManagerServer.TASK_STATES["RECEIVE_BAG"]:
                rospy.loginfo("[INFO] Picking bag...")
                self.subtask_manager["hri"].speak("Please, hand me the bag")
